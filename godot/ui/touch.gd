...
var _font: Font
var _own := {}
var _flash := {"left": 0.0, "right": 0.0, "up": 0.0, "dive": 0.0, "special": 0.0}
...
## O especial é um toque pensado, não um reflexo: botão menor, acima do pulo.
func _rr(act: String) -> float:
	return _r() * (0.8 if act == "special" else 1.0)
...
func _pos(act: String) -> Vector2:
	...
	match act:
		...
		"special":
			return Vector2(size.x - sx - r, bottom - r * 2.45)
		"dive":
...
func zone(p: Vector2) -> String:
	...
	var best := "up"
	var bd: float = p.distance_to(_pos("up")) / JUMP_PULL
	var da: float = p.distance_to(_pos("dive"))
	if da < bd:
		best = "dive"
		bd = da
	var ds: float = p.distance_to(_pos("special")) / SPECIAL_PULL
	if ds < bd:
		best = "special"
	return best
...
func _draw() -> void:
	...
	_draw_btn("special", "↯", "SPECIAL", Color(1.0, 0.80, 0.25), held)
...
func _draw_zone_hint(held: Array) -> void:
	for k in ["left", "right", "dive", "up", "special"]:
...
func _draw_btn(act: String, icon: String, label: String, col: Color, held: Array) -> void:
	...
	var c := col
	var ready: bool = act == "special" and charge >= 1.0
	if act == "special" and not ready:
		c = Color(0.62, 0.58, 0.50)
	if ready:
	...
	if act == "special" and not ready:
		draw_arc(p, r - 11.0, -PI * 0.5, -PI * 0.5 + TAU * charge, 48,
			Color(1.0, 0.80, 0.25, 0.85), 5.0, true)
	var fs := int(r * 0.78)
	...
	var t := "READY!" if ready else label
	if t == "":
		return
